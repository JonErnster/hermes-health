import {makeOverlay,type Overlay} from './overlay';
...
interface Props {atlas:Atlas;state:SceneState;onSelect:(id:string)=>void;onProgress:(n:number)=>void;onError:(s:string)=>void;overlay?:Overlay;onLandmark?:(id:string)=>void}
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError,overlay,onLandmark}:Props){
 const host=useRef<HTMLDivElement>(null),latest=useRef(state),select=useRef(onSelect);
 const overlayRef=useRef(overlay),landmarkRef=useRef(onLandmark);overlayRef.current=overlay;landmarkRef.current=onLandmark;
 ...
 useEffect(()=>{
  ...
  const fit=(view:string,extent=0)=>{
   ...
   if(overlayRef.current&&!extent){const height=2.35,usable=Math.max(.3,(el.clientHeight-180)/el.clientHeight);const d=Math.max(height/usable,1.85/camera.aspect)/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2)));const dir=view==='front'?new T.Vector3(0,.01,1):view==='back'?new T.Vector3(0,.01,-1):view==='side'?new T.Vector3(1,.01,0):new T.Vector3(.35,.06,1).normalize();controls.target.set(0,.92,0);camera.position.copy(controls.target).addScaledVector(dir,d);controls.update();dirty=true;return;}
  ...
  const up=(e:PointerEvent)=>{
   ...
   if(decoration?.group.visible){const hit=raycaster.intersectObjects(decoration.group.children).find(h=>h.object.userData.landmarkId);if(hit){landmarkRef.current?.(hit.object.userData.landmarkId);return;}}
  ...
  let renderedOverlay:Overlay|undefined,decoration:ReturnType<typeof makeOverlay>|undefined;
  const clock=new T.Clock();let lastExtent=-1;
  const animate=()=>{
   if(disposed)return;frame=requestAnimationFrame(animate);const dt=Math.min(clock.getDelta(),.05),s=latest.current;
   if(renderedOverlay!==overlayRef.current){
    if(decoration){scene.remove(decoration.group);decoration.dispose();decoration=undefined;}
    renderedOverlay=overlayRef.current;
    if(renderedOverlay){decoration=makeOverlay(renderedOverlay,id=>landmarkRef.current?.(id));scene.add(decoration.group);decoration.labels.forEach(l=>el.appendChild(l.node));}
    const dark=renderedOverlay?.dark;renderer.setClearColor(dark?'#101d27':'#f2f3f3');
    (ground.material as T.MeshStandardMaterial).color.set(dark?'#101d27':'#d5d9dc');
    mats.forEach((m,id)=>{const opacity=renderedOverlay?.opacity??1;const mat=m as T.MeshStandardMaterial;mat.opacity=id==='integumentary'?opacity*.15:opacity;mat.transparent=mat.opacity<1;mat.depthWrite=mat.opacity>=1;});dirty=true;
   }
   if(decoration){decoration.group.visible=s.explode<.05&&!s.isolate;decoration.labels.forEach(l=>{const v=l.position.clone().project(camera);l.node.hidden=!decoration!.group.visible||!renderedOverlay?.labels||(renderedOverlay.points.length>18&&l.node.getAttribute('aria-pressed')!=='true')||v.z< -1||v.z>1;l.node.style.left=((v.x+1)*el.clientWidth/2)+'px';l.node.style.top=((1-v.y)*el.clientHeight/2)+'px';});}
  ...
  return()=>{if(decoration){scene.remove(decoration.group);decoration.dispose();}disposed=true;abort.abort();cancelAnimationFrame(frame);observer.disconnect();controls.dispose();geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());scene.traverse(o=>{if(o instanceof T.Mesh&&!geometries.includes(o.geometry)){o.geometry.dispose();const ms=Array.isArray(o.material)?o.material:[o.material];ms.forEach(m=>m.dispose());}});env.dispose();partTexture.dispose();selectionTexture.dispose();markerGeometry.dispose();markerMaterial.dispose();hover.remove();renderer.dispose();renderer.domElement.remove();};
 },[atlas]);
